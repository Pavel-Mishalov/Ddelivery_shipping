...
var map_edit_button = ( jQuery('label[for="shipping_method_0_mis_ddelivery_pickup_method"]').html() == 'Самовывоз' ) ? 'Выбрать точку' : 'Изменить точку';

    if( jQuery('label[for="shipping_method_0_mis_ddelivery_pickup_method"]').html() == 'Самовывоз' ){
      jQuery("#shipping_method_0_mis_ddelivery_pickup_method").change( function(){ set_map_pickup_point() } );
    }
    jQuery('label[for="shipping_method_0_mis_ddelivery_pickup_method"]').append('<a id="get_map_point"> ' + map_edit_button + '</a>');
    //jQuery("#shipping_method_0_mis_ddelivery_pickup_method").removeAttr( 'checked' );
    jQuery("#get_map_point").click( function(){ set_map_pickup_point() } );
    function set_map_pickup_point(){
    ...
    jQuery.ajax({
      ...
      beforeSend: function(){
          jQuery('#map').html('');
          jQuery('#png_before').css('display', 'block');
      ...
      success: function(res){
        ...
        console.log(res);
        if( res.length == 0 ){
          alert('Просим прошения. Для Вашего заказа нет точек продаж');
          jQuery('#map').html('');
          jQuery('#png_before').css('display', 'none');
          ...
          HintLayout = ymaps.templateLayoutFactory.createClass( "<div class='my-hint'>" +
            ...
            "<div class='my-hint2'>{{ properties.price }}<span class='woocommerce-Price-currencySymbol'>₴</span></div>" +
            "</div>", {
         ...
         MyBalloonLayout = ymaps.templateLayoutFactory.createClass(
            '<div class="popover top ballon">' +
                '<div class="arrow ballon-header"><span>{{properties.name}}</span><a class="close" href="#">&times;</a>' +
                '</div>' +
                '<div class="popover-inner ballon-content">' +
            ...
            MyBalloonContentLayout = ymaps.templateLayoutFactory.createClass(
              '<table class="shop-setting">'+
                '<tr>'+
                  '<td class="mis-key">Компания: </td>'+
                  '<td class="mis-value">{{properties.company}}</td>'+
                '</tr>'+
                '<tr>'+
                  '<td class="mis-key">Дата доставки: </td>'+
                  '<td class="mis-value">{{properties.date}} {{properties.day}}</td>'+
                '</tr>'+
                '<tr>'+
                  '<td class="mis-key">Тип пункта: </td>'+
                  '<td class="mis-value">{{properties.type}}</td>'+
                '</tr>'+
                '<tr>'+
                  '<td class="mis-key">Вариант оплаты: </td>'+
                  '<td class="mis-value">{{properties.pay_method}}</td>'+
                '</tr>'+
                '<tr>'+
                  '<td class="mis-key">Адрес: </td>'+
                  '<td class="mis-value">{{properties.address}}</td>'+
                '</tr>'+
                '<tr>'+
                  '<td class="mis-key">Метро: </td>'+
                  '<td class="mis-value">{{properties.metro}}</td>'+
                '</tr>'+
                '<tr>'+
                  '<td class="mis-key">Расписание работы: </td>'+
                  '<td class="mis-value">{{properties.schedule}}</td>'+
                '</tr>'+
                '<tr>'+
                  '<td class="mis-key">Подробнее: </td>'+
                  '<td class="mis-value">{{properties.all}}</td>'+
                '</tr>'+
                '<tr>'+
                  '<td class="mis-pickup-cost" colspan="2"><div class="btn-price"><p class="pickup-price">{{properties.price}}<span class="woocommerce-Price-currencySymbol">₴</span></p><button class="mis-btn" onclick="get_point({{properties.price}}, \' ( {{properties.address}}. Компания: {{properties.company}} )\');">Заберу здесь</button></td>'+
                '</tr>'+
              '</table>'
            )
            ...
            for (var i = 0, len = res.length; i < len; i++) {
              ...
              var typew = ( res[i]['type'] == 1 ) ? 'Ячейка' : 'Живой пункт';
              var nal = ( res[i]['is_cash'] ) ? 'Наличными' : '';
              var cart = ( res[i]['is_card'] ) ? 'Картой' : '';
                geoObjects[i] = new ymaps.Placemark([ res[i]['latitude'], res[i]['longitude'] ],
                {
                  ...
                  metro: res[i]['metro'],
                  schedule: res[i]['schedule'].replace(/\,/g, ', '),
                  all: res[i]['indoor_place']
                }, {
        ...
        setTimeout( "jQuery('#png_before').css('display', 'none')", 2000);
      }
    });
      }
